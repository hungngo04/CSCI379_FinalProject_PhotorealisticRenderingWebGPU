...
export default class VolumeRenderingSimpleObject extends RayTracingObject {
  constructor(device, canvasFormat, camera) {
    ...
    this._camera = camera;
    // Add trace type property (0: MIP, 1: DRR, 2: DepthEncoding, 3: SpectralEdgeDetection)
    this._traceType = 3; // Default to spectralEdgeDetection
  }

  // Add method to set trace type
  setTraceType(traceType) {
    this._traceType = traceType;
    // Update buffer when trace type changes
  ...
  async createGeometry() {
    ...
    this._materialBuffer = this._device.createBuffer({
      label: "Material Params " + this.getName(),
      size: 8 * Float32Array.BYTES_PER_ELEMENT, // 32 bytes (required alignment)
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    // Set initial material values (e.g., white albedo, roughness 0.5, metallic 0.0)
    const materialParams = new Float32Array([1.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 0.0]); // Red
    this._device.queue.writeBuffer(this._materialBuffer, 0, materialParams);

    // --- Light buffer: add for photorealistic rendering ---
    this._lightBuffer = this._device.createBuffer({
      label: "Light Params " + this.getName(),
      size: 8 * Float32Array.BYTES_PER_ELEMENT, // 32 bytes (vec3 pos, vec3 color, intensity, pad)
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    // Set initial light values (position, color, intensity, pad)
    const lightParams = new Float32Array([
      0.0, 0.0, 2.0, // position (above scene)
